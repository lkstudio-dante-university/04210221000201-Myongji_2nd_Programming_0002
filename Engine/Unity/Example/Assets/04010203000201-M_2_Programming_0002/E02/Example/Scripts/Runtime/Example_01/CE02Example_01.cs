...
/*
 * Unity 란?
 * - 게임을 비롯한 다양한 비주얼 프로그램을 제작 할 수 있게 여러 기능을 제공하는 멀티 플랫폼 게임
 ...
 * 
 * 따라서 컴포넌트를 조합해서 특정 역할을 하는 대상을 표현하는 것이 가능하다. (즉, Unity 는 
 ...
 * 
 * 따라서 Unity 는 게임 객체와 더불어 여러 컴포넌트를 제공하며 여러 컴포넌트를 게임 객체에 
 * 추가함으로서 특정 대상을 표현하는 것이 가능하다. (즉, 게임 객체는 단순히 컴포넌트를 담기 위한 
 * 그릇에 불가하다는 것을 알 수 있다.)
 * 
 * 단, Unity 가 제공해주는 컴포넌트만으로는 다양한 목적에 맞는 프로그램을 제작하는 것이 불가능하기 
 * 때문에 Unity 는 사용자가 직접 구현 할 수 있는 컴포넌트를 제공하며 이를 스크립트 컴포넌트라고 

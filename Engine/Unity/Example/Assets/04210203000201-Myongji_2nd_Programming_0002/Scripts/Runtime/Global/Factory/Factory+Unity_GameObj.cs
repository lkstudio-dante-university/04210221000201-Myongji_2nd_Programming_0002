...
public static partial class Factory
{
	...
	{
		return Factory.CreateGameObj(a_oName,
			a_oGameObj_Parent, Vector3.zero, Vector3.one, Vector3.zero, a_bIsStay_WorldState);
	}
...
public static partial class Factory
{
	...
	{
		return Factory.CreateGameObj_Clone(a_oName,
			a_oPath_Prefab, a_oGameObj_Parent, Vector3.zero, Vector3.one, Vector3.zero, a_bIsStay_WorldState);
	}
	...
	{
		return Factory.CreateGameObj_Clone(a_oName,
			Resources.Load<GameObject>(a_oPath_Prefab), a_oGameObj_Parent, Vector3.zero, Vector3.one, Vector3.zero, a_bIsStay_WorldState);
	}
	...
	{
		return Factory.CreateGameObj_Clone(a_oName,
			a_oPrefab_Origin, a_oGameObj_Parent, Vector3.zero, Vector3.one, Vector3.zero, a_bIsStay_WorldState);
	}
		...
		GameObject a_oPrefab_Origin, GameObject a_oGameObj_Parent, Vector3 a_stPos, Vector3 a_stScale, Vector3 a_stRotate, bool a_bIsStay_WorldState = false)
	{
		 ...
		 * 컴포넌트를 설정해주지 않으면 아무런 역할도 하지 않는다. (즉, 새로운 게임 객체를 생성하기
		 * 위해서는 객체를 생성 후 관련 컴포넌트를 추가 및 설정해 줄 필요가 있다.)
		 * 

...
/*
 * 델리게이트란?
 * - 메서드를 데이터처럼 제어 할 수 있는 기능을 의미한다. (즉, 델리게이트를 활용하면 메서드를 특정 메서드에 입력
 * 데이터로 전달하거나 메서드를 반환하는 것이 가능하다.)
 * 
 * 또한, 델리게이트를 통해 메서드를 호출하는 것이 가능하다. (즉, 메서드의 이름을 명시적으로 지정하지 않고 특정 
 * 메서드를 호출하는 것이 가능하다.)
 * 
 * C# 델리게이트 선언 방법
 ...
 * 
 * C# 델리게이트 종류
 * - Func		<- 반환 데이터가 존재하는 메서드 제어
 * - Action		<- 반환 데이터가 없는 메서드 제어
 * 
 * C# 은 Func 과 Action 델리게이트를 제공하기 때문에 명시적으로 델리게이트를 선언하지 않아도 델리게이트를 사용하는
 * 것이 가능하다. (즉, 명시적으로 델리게이트를 선언하는 것은 가독성을 위해 좀 더 의미있는 이름을 부여하는 것이라는
 * 것을 알 수 있다.)
 * 
 * Ex)
 * int SomeMethodA(int a_nValA, int a_nValB)
 * {
 *		// Do Something
 * }
 * 
 * void SomeMethodB(int a_nValA, int a_nValB, int a_nValC)
 * {
 *		// Do Something
 * }
 * 
 * Func<int, int, int> oFunc = SomeMethodA;
 * Action<int, int, int> oAction = SomeMethodB;
 * 
 * 위와 같이 Func 델리게이트는 반환 값이 존재하는 메서드를 제어하는 것이 가능하며 Action 델리게이트는 반환 값이 없는
 * 메서드를 제어하는 것이 가능하다. (즉, Func 델리게이트 제네릭 형식 인자의 가장 마지막은 반환 형을 나타낸다는 것을
 * 알 수 있다.)
 */
namespace Example._04210221000201_Myongji_2nd_Programming_0002.E01.Example.Classes.Runtime.Example_17
{
	/**
	 * Example 17
	 */

...
/*
 * 자료형이란?
 * - 데이터를 해석하기 위한 방법을 의미한다. (즉, 동일한 형태의 데이터라고 하더라도 자료형에 의해 서로 다른 결과를
 * 만들어 낼 수 있다.)
 * 
 ...
 * 받지 않는다는 것을 알 수 있다.)
 * 
 * 또한, 값 형식 자료형의 데이터는 시스템에 의해서 메모리가 관리되기 때문에 참조 형식 자료형의 데이터보다 메모리
 * 관리가 수월하다는 장점이 존재한다.
 * 
 * 참조 형식 자료형이란?
 ...
 * 
 * Ex)
 * int nVal01 = 0;		<- 첫 문자가 숫자가 아니기 때문에 컴파일 에러가 발생하지 않는다.
 * int 01nVal = 0;		<- 첫 문자가 숫자이기 때문에 컴파일 에러가 발생한다.
 */
namespace E01.Example.Classes.Example_02
{
	/**
	 * Example 2
	 */
	internal class CE01Example_02
	{
		...
		{
			/*
			 * C# 의 모든 자료형은 클래스이기 때문에 . (맴버 지정 연산자) 를 통해 하위에 존재하는 여러 기능을
			 * 사용하는 것이 가능하다.
			 * 
